...
This field is an array of links. The order in which the links are rendered can be controlled either in Admin, or using the [#sort_list](Propeller-Helpers.md#sort_list) propeller.
...
Example markup using the [#sort_list](Propeller-Helpers.md#sort_list) propeller:
```
<div class="additional-links">
...
Aside from the image, there are optional text inputs similar to the [link](#field-type-link) field, which are title, subtitle, and caption. The url is also accessible, but is not modifiable as it is the url returned from Cloudinary.
...
This field is an array of images. The order in which the images are rendered can be controlled either in Admin, or using the [#sort_list](Propeller-Helpers.md#sort_list) propeller.
...
## Field Type: Date
This field renders as a unix timestamp by default. To format it, use the [format_date](Propeller-Helpers.md#format_date) helper.
...
The checkbox is useful for adding flags that can be used in combination with propellers, such as [#if](Propeller-Helpers.md#if) or [#unless](Propeller-Helpers.md#unless), in order to conditionally render content.
...
## Field Type: Number
While this field is a regular text input, it can still be used in place of the [text](#field-type-text) field to be more semantically correct.
