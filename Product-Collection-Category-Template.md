# Product Collection `category.html` Template:
The following properties and fields can be rendered on the category.html template:

## Property: `params`
This property is for pagination.

The following fields are available from the category.html template:
```
page
limit
offset
...
Example markup using the `{{create_page_query}}` helper with fields from `params` to create page navigation links:
```
<li><a href="{{params.fetch_prev}}">Previous Page</a></li>
<li><a href="{{params.fetch_next}}">Next Page</a></li>
```

Example Output HTML:
```
<li><a href="?page=1&amp;limit=50&amp;sort=id&amp;order=asc">Previous Page</a></li>
<li><a href="?page=2&amp;limit=50&amp;sort=id&amp;order=asc">Next Page</a></li>
```

Example Output Query Strings:
```
/mammals/categories/essentials?page=1&limit=50&sort=id&order=asc
/mammals/categories/essentials?page=2&limit=50&sort=id&order=asc
```
...
```
{{#each items}}
<p>{{format_date updated_at "f" "us"}}</p>
{{/each}}
```

Example Output:
```
<p>Thursday, June 01, 2017 4:43 AM</p>
```
...
## Property: `collection`
The following are examples of how to render the available properties/fields on collection:
